...
if (isset($_POST['dni']) && isset($_POST['nombre'])) {
    ...
    if (!($sentencia = $mysqli->prepare("UPDATE alumnos SET nombre=?,email=?,codmatricula=?,ciclo=?  WHERE dni=?"))) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }
    
    /* Sentencia preparada, etapa 2: vinculación y ejecución */
    if (!$sentencia->bind_param("sssss", $nombre,$email,$codmatricula,$ciclo, $dni)) {
